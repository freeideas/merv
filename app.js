...
// Default 3.8.1: its onnxruntime-web (1.22.0-dev, Apr 2025) includes the
// MatMulNBits WebGPU prefill-shader race-condition fix (onnxruntime PR #23663,
// merged 2025-02-14). 3.3.3's ort-web (1.21.0-dev.20250206, Feb 6 2025) predates
// it by 8 days -> correct q4 output on CPU/WASM but garbled on WebGPU.
// Overridable for A/B testing across releases, e.g. ?tjs=3.3.3 or ?tjs=4.0.0.
const _tjs = new URLSearchParams(location.search).get("tjs");
const TJS_VERSION = _tjs && /^[\w.\-]+$/.test(_tjs) ? _tjs : "3.8.1";
